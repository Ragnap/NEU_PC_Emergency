...
public class ProblemDB extends Database {
    ...
    public ProblemData getProblem(int id) throws SQLException {
        PreparedStatement prep = connection.prepareStatement("select * from web.problem where problem.id = ?");
        prep.setInt(1, id);
        prep.execute();
        ResultSet resultSet = prep.executeQuery();
        if (resultSet.next()) {
            ...
            String system = resultSet.getString("system");
            String solution = resultSet.getString("solution");
            return new ProblemData(id, username, time, describe, model, system, solution);
        } else
            return null;
    }
}
